...
base = Path("/home/ralph/insurance-governance")
...
lines = [
    ...
    "from pathlib import Path",
    "",
    "# Install pydantic v2 + typing_extensions into an isolated dir so they beat bundled v1",
    "import tempfile as _tf",
    "dep_dir = _tf.mkdtemp(prefix='ig_deps_')",
    "subprocess.run([sys.executable, '-m', 'pip', 'install',",
    "    '--target', dep_dir, '--quiet', '--no-deps',",
    "    'pydantic==2.8.2', 'pydantic-core==2.23.4'], check=True)",
    "subprocess.run([sys.executable, '-m', 'pip', 'install',",
    "    '--target', dep_dir, '--quiet', '--no-deps',",
    "    'annotated-types>=0.6.0'], check=True)",
    "# Insert dep_dir at position 0 so our pydantic v2 beats any bundled v1",
    "sys.path.insert(0, dep_dir)",
    "# Also install remaining deps system-wide (polars, jinja2, pytest, scikit-learn)",
...
lines.extend([
    "",
    "# Add to Python path (dep_dir first so pydantic v2 beats system pydantic v1)",
    "sys.path.insert(0, str(tmpdir))",
    "sys.path.insert(0, dep_dir)",
    "",
    "import insurance_governance",
    "",
    "# Run pytest — include dep_dir in PYTHONPATH so subprocess also gets pydantic v2",
    "# and skip fairness-dependent tests (insurance-fairness not installed)",
    "env = dict(os.environ)",
    "env['PYTHONPATH'] = dep_dir + ':' + str(tmpdir) + ':' + env.get('PYTHONPATH', '')",
    "# Skip fairness test files by excluding them explicitly",
