"""
Run test_discrimination_coverage.py on Databricks serverless compute.
"""
...
WORKSPACE_DIR = "/Workspace/Shared/insurance-governance-disc-cov"
...
# MAGIC %pip install polars numpy pytest
...
LOCAL_DIR = tempfile.mkdtemp(prefix="ins_gov_disc_")
...
result = subprocess.run(
    [sys.executable, "-m", "pytest",
     LOCAL_DIR + "/tests/test_discrimination_coverage.py",
...
NOTEBOOK_PATH = f"{WORKSPACE_DIR}/_run_discrimination_coverage"
...
run_response = w.jobs.submit(
    run_name="insurance-governance-discrimination-coverage",
    tasks=[
        jobs.SubmitTask(
            task_key="pytest",
    ...
    environments=[
        jobs.JobEnvironment(
            ...
            spec=jobs.compute.Environment(
                client="2",
                dependencies=["polars", "numpy", "pytest"],
            ),
        )
    ],
)
